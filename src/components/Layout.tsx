      ...
      <header className="fixed top-0 w-full bg-white shadow-sm z-50">
        <nav className="container mx-auto px-3 sm:px-4 py-3 sm:py-4">
          <div className="flex items-center justify-between gap-3 sm:gap-4">
            ...
            <div className="flex items-center gap-2 sm:gap-2 overflow-x-auto scrollbar-hide flex-1 lg:flex-initial justify-center lg:justify-start">
              {navLinks.map((link) => (
                <Link
                  key={link.path}
                  to={link.path}
                  className={`px-3 sm:px-3 lg:px-5 py-2 sm:py-2 lg:py-2.5 text-sm sm:text-sm lg:text-base whitespace-nowrap rounded-lg transition font-medium ${
